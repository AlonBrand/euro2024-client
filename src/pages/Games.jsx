...
import { eighthGames } from '../constants/games';
...
function Games(prop) {
  ...
  const availableGames = eighthGames.filter((game) => {
    const gameEndTime = new Date(game.date.getTime() + 180 * 60000);
    return new Date() <= gameEndTime
  });

  const paginatedData = showAllGames ? eighthGames.slice((page - 1) * itemsPerPage, page * itemsPerPage) : availableGames.slice((page - 1) * itemsPerPage, page * itemsPerPage);
  const gamesCount = showAllGames ? eighthGames?.length : availableGames?.length;
